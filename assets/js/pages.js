...
// class PageTransitionRegion extends Marionette.Region {
//   initialize(options) {
...
var PageView = Marionette.View.extend({
  template: _.template($("#page-tpl").html())
});
var PageTransitions = function () {
  ...
  function init() {
    mainView.showChildView("main", new PageView({
      className: "pt-page pt-page-1 pt-page-current"
    }));
    $("#dl-menu").dlmenu({
      animationClasses: {
        in: "dl-animate-in-2",
  ...
  function nextPage(options) {
    ...
    var page = new PageView({
      className: "pt-page pt-page-" + (current + 1)
    });
    var region = mainView.getRegion("main");
    region.animation.inClass = inClass;
    region.animation.outClass = outClass;
    region.show(page);
  }
  init();
  return {
